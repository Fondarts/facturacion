...
export default function Facturar() {
  ...
  async function handleDeleteCliente(id: string, e: React.MouseEvent) {
    e.stopPropagation();
    if (!confirm(t('facturar.confirmDeleteCliente'))) return;
    try {
  ...
  async function handleDeleteEmisor(id: string, e: React.MouseEvent) {
    e.stopPropagation();
    if (!confirm(t('facturar.confirmDeleteEmisor'))) return;
    try {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">{t('facturar.title')}</h1>
          <p className="text-slate-400">{t('facturar.subtitle')}</p>
        </div>
      </div>
